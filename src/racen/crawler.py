...
def _build_session(timeout: int, ua: str) -> requests.Session:
    s = requests.Session()
    retry = Retry(
        total=3,
        backoff_factor=0.5,
...
def _should_visit(url: str, origin: str, cfg: CrawlConfig) -> bool:
    ...
    if cfg.include_patterns:
        ...
        ok = any(pat.lower() in url.lower() for pat in cfg.include_patterns if pat)
        if not ok:
            return False
    return True
...
def crawl(cfg: CrawlConfig) -> List[str]:
    ...
    if not parsed.netloc.startswith("www."):
        candidates.append(
            urllib.parse.urlunsplit((parsed.scheme, "www." + parsed.netloc, parsed.path, parsed.query, ""))
        )
    if parsed.scheme == "https":
        candidates.append(
            urllib.parse.urlunsplit(("http", parsed.netloc, parsed.path, parsed.query, ""))
        )
    start_resolved = None
    session = _build_session(cfg.timeout, cfg.user_agent)
    ...
    if not start_resolved:
        logger.warning(f"Unable to resolve start URL after probes: {start}")
        return []
