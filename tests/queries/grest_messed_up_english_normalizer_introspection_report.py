...
QUERIES_TXT = ROOT / "tests" / "queries" / "grest_messed_up_english_queries.txt"
OUT_MD = ROOT / "tests" / "queries" / "grest_messed_up_english_normalizer_introspection.md"


def _load_queries(path: Path) -> List[Tuple[str, int]]:
    """Load messed-up English queries from a text file.
...
def _safe_normalize(q: str) -> str:
    """Run the LLM normalizer on a query and return the normalized text.
...
def run() -> int:
    """Generate an introspection report for messed-up English normalization.

    For each query in grest_messed_up_english_queries.txt, this script:
    - Records the original messy text.
    ...
    lines: List[str] = []
    lines.append("# GREST Messed-Up English Normalization Introspection\n")
    ...
    for idx, (q, k) in enumerate(pairs, 1):
        ...
        lines.append(f"\n## [{idx}] Query\n")
        lines.append(f"**Original (messy) text:** {q}\n")
    ...
    print(f"Wrote messed-up English normalization introspection report to {OUT_MD}")
    return 0
